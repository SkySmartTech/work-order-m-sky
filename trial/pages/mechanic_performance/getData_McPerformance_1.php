<?php
    ...
    else if($strFuncType === "funGetData_CheckInAllocateTimeChart") //------------- funUpdateEventLog --------------------
    {
        ...
        {       
            ...
            $sqlString = "SELECT WorkOrderNo,CheckInUserDateTime, CheckOutUserDateTime
                ...
                AND DATE(CheckInUserDateTime) = CURDATE()
    ...
    /*
    else if($strFuncType === "funGetData_AllocateChart") //------------- funUpdateEventLog --------------------
    {
        $strEmpEPF         = $num[1]; 
        //$strStarDate       = $num[2];
        //$strEndDate        = $num[3];
        ...
        try 
        {            	 
            //WorkOrderNo,AllocatedBy,AllocatedUser,AllocatedServerDateTime,AllocatedUserStartDateTime,AllocatedUserEndDateTime,DeAllocatedBy,DeAllocatedDateTime,Status 
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
            $sqlString = "SELECT WorkOrderNo,AllocatedUserStartDateTime,AllocatedUserEndDateTime)
                FROM tblwo_allocatedusers 
                WHERE AllocatedUser = :allousr
                AND DATE(AllocatedUserStartDateTime) = CURDATE()
                AND Status ='Active'
                ORDER BY AllocatedTime ASC";
            ...
            foreach($result as $row)
            {   
                $ReturnData_ary[$i][0] = $row['Allocated Time'];
                $ReturnData_ary[$i][1] = $row['WorkOrderNo'];
                $ReturnData_ary[$i][2] = $row['CheckInUserDateTime']; 
                $ReturnData_ary[$i][3] = $row['CheckOutUserDateTime'];
                $i++;
