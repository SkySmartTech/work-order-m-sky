...
<script>
    ...
    {      
        ...
        // Set the value of 'id_startdate' to 7 days ago
        document.getElementById('id_startdate').valueAsDate = new Date(Date.now() - (3600 * 1000 * 24 * 7));

        // Get the current date
        var currentDate = new Date();

        // Add 8 days (in milliseconds) to make it 7 days after the start date
        var eightDays = 0 * 24 * 60 * 60 * 1000; // 8 days * 24 hours * 60 minutes * 60 seconds * 1000 milliseconds
        var endDate = new Date(currentDate.getTime() + eightDays);

        // Subtract one day (in milliseconds) from endDate
        var oneDay = 24 * 60 * 60 * 1000; // 24 hours * 60 minutes * 60 seconds * 1000 milliseconds
        endDate = new Date(endDate.getTime() - oneDay);

        // Set the value of 'id_enddate'
        document.getElementById('id_enddate').valueAsDate = endDate;
