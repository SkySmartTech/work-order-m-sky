<?php
    ...
    if ($strFuncType === "funGetData_Total_checking"){
        $strStartDate    = $num[1];
        $strEndDate      = $num[2];
        $strDepartment   = $num[3];
        $strCategory     = $num[4];
        ...
        $whereClause = "wo.ClosedDateTime IS NOT NULL AND State < 6 AND DATE(wo.CreatedDateTime) BETWEEN :sdate AND :edate";
        ...
        try {
            $sqlString = "
                SELECT 
                    SUM(TIMESTAMPDIFF(MINUTE, CheckInUserDateTime, CheckOutUserDateTime)) AS Time_Duration 
                FROM
                    tblwo_event AS wo
            ...
            if (empty($result)) {
                ...
                foreach($result as $row) {
                    $ReturnData_ary[0][] = number_format($row['Time_Duration'] / 60, 1);
    ...
    else if ($strFuncType === "funGetData_Total_Allocated"){
        $strStartDate    = $num[1];
        $strEndDate      = $num[2];
        $strDepartment   = $num[3];
        $strCategory     = $num[4];
        ...
        if ($strMechanic !== "All") {
            $whereClause .= " AND aiu.AllocatedUser='" . $strMechanic . "'";
        }
        
        try {
            $sqlString = "
                SELECT 
                    SUM(TIMESTAMPDIFF(MINUTE, AllocatedUserStartDateTime, AllocatedUserEndDateTime)) AS Time_Duration 
                FROM
                    tblwo_event AS wo 
                    INNER JOIN tblwo_allocatedusers AS aiu ON wo.WorkOrderNo = aiu.WorkOrderNo
                WHERE 
                    " . $whereClause  ;
    ...
    else if($strFuncType === "funGetData_PieChart1") {
        $strStartDate    = $num[1];
        $strEndDate      = $num[2];
        $strDepartment   = $num[3];
        $strCategory     = $num[4];
    ...
    else if($strFuncType === "funGetData_BarChart1") //------------- funUpdateEventLog --------------------
    {
        ...
        try {             
            $sqlString = "
                SELECT 
                    wo.WoDepartment,
                    SUM(CASE WHEN wo.WoStatus IN ('Auto-Verified', 'Verified' ,'Close') THEN 1 ELSE 0 END) AS close_count,
                    SUM(CASE WHEN wo.WoStatus = 'Inprogress' THEN 1 ELSE 0 END) AS inprogress_count,
                    SUM(CASE WHEN wo.WoStatus = 'New' THEN 1 ELSE 0 END) AS New_count
                FROM 
                    tblwo_event AS wo
                INNER JOIN 
            ...
            {   
                ...
                $ReturnData_ary[3][$i] = $row['New_count'];
