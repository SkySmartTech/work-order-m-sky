...
<script>
    ...
    function drawGooleChart() 
    {
        ...
        $.post('getData_McPerformance.php', { userpara: DataAry }, function(json_data2) 
        {
            ...
            else
            {
                ...
                dataTable.addRows([
                     ['Allocate Time', '', new Date(2024, 3, 6, 7, 0, 0), new Date(2024, 3, 6, 7, 0, 1)],
                     ['Allocate Time', '', new Date(2024, 3, 6, 19, 0, 0), new Date(2024, 3, 6, 19, 0, 1)],
                     ['Check In Time', '', new Date(2024, 3, 6, 7, 0, 0), new Date(2024, 3, 6, 7, 0, 1)],
                     ['Check In Time', '', new Date(2024, 3, 6, 19, 0, 0), new Date(2024, 3, 6, 19, 0, 1)],
                   ]);
    ...
    function funViewReport() 
    {
        ...
        $.post('getData_McPerformance.php', { userpara: DataAry }, function(json_data2) 
        {
            ...
            if(res.Status_Ary[0] === "true")   // No data found, insert new record
            {
                ...
                {
                    intTmp = i + 1; 
                    let strWoCategory   = res.Data_Ary[i][5];
                    let strDescription  = "";
                    if(strWoCategory === "BreakDown")
                    {
                        //alert("Breakdown");
                        strDescription = res.Data_Ary[i][7] + " (" +res.Data_Ary[i][8] + ")";
                    }
                    else if(strWoCategory === "PlanMaintenance")
                    {
                        //alert("PlanMaintenance");
                        strDescription = res.Data_Ary[i][1];
                    }  
                    ...
                    {
                        //alert("OtherProject");
                        strDescription = res.Data_Ary[i][6] + " (" +res.Data_Ary[i][1] + ")";
                    }
                    ...
                    dtbl1.row.add([intTmp.toString(), res.Data_Ary[i][0],strDescription,res.Data_Ary[i][2],res.Data_Ary[i][3], res.Data_Ary[i][4]]).draw(false);
                } 
                //---------- Insert Table Footer -------------------------        
    ...
    function drawPieCharts() 
    {
        ...
        $.post('getData_McPerformance.php', { userpara: DataAry }, function(json_data2) 
        {
            ...
            //{             
                ...
                const AryAllocate_Duration  = [0, 100];
                const AryAllocate_Colors    = ["#8F3985", "#FECF6A"];                
                                
                var intValue = parseInt(res.Data_Ary[0][0], 10);
                AryAllocate_Duration[0] = ((intValue * 100)/720).toFixed(1);              
               
                if(intValue < 720)
                {                    
                    AryAllocate_Duration[1] = (100 - (intValue * 100)/720).toFixed(1);
                }
                else
                {
                ...
                AryCheckIn_Duration[0] = ((intValue * 100)/720).toFixed(1);
                if(intValue < 720)
                {                    
                    AryCheckIn_Duration[1] = (100 - (intValue * 100)/720).toFixed(1);
                }
                else
                {
    ...
    //-------------------- Draw Pie CHart Allocation Chart --------------------------------------    
    function drawPieCharts_Allocate() 
    {
        let intDebugEnable = 0;        
        if(intDebugEnable === 1) alert("drawPieCharts_Allocate"); 
        const DataAry = [];
        //-------------------- Update CheckIn Efficency Chart-----------------------------------------------------        
        DataAry[0] = "funGetData_AllocatePieChart";        // Table Name
        DataAry[1] = document.getElementById("id_SelMechanics").value;
        DataAry[2] = document.getElementById("id_lbl_sdate").innerHTML;
        DataAry[3] = document.getElementById("id_lbl_edate").innerHTML;
        if(intDebugEnable === 1) alert("DataAry :" + DataAry);
        $.post('getData_McPerformance.php', { userpara: DataAry }, function(json_data2) 
        {
            ...
            //{             
                ...
                for (const interval of res.Data_Ary) 
                {
                ...
                if(intDebugEnable === 1) alert("Total allocate duration: " + totalDuration + " minutes");
      
                const AryAllocate_Lable     = ["Allocated Duration", "Idle Duration"];
                const AryAllocate_Duration  = [0, 100];
                const AryAllocate_Colors    = ["#8F3985", "#FECF6A"];                
                                
                var intValue = parseInt(totalDuration);
                AryAllocate_Duration[0] = ((intValue * 100)/720).toFixed(1);              
